...
const currentConfig = sceneConfigurations[configIndex];
...
let percent = [];
currentConfig.models.forEach((modelData, i) => {
    loader.load(modelData.path, (geometry) => {
        ...
        if (loadedMeshes.length === currentConfig.models.length) {
        ...
        let p = percent.reduce((accumulator, current) => accumulator + current, 0) / currentConfig.models.length;
...
function animate(time) {
    ...
    if (autoRotate) {
        loadedPivots.forEach(pivot => {
            pivot.rotation.z += currentConfig.autoRotateSpeed ?? 0.01;
        });
    }
